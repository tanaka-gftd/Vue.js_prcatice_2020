//数値を通貨書式「#,###,###」に変換するフィルター
/* 汎用的なフィルターなので、グローバルスコープに登録する */
/* グローバルスコープに登録するフィルターの定義は、フィルターを利用する側のコンポーネントよりも先に記述する必要がある */
...
var app = new Vue ({
  ...
  /* el：コンポーネントのインスタンスをどのHTML要素に結び付けるかを定義する */
  el: '#app',
  
  /* data：コンポーネントの保持するデータを定義する */
  data: {
  ...
  /* computed：コンポーネントの持つ算出プロパティを定義する 
     算出プロパティ：アプリケーションのデータに基づいて、何らかの加工を行った結果を返すプロパティ。 返された結果は再利用され、リアクティブな依存関係の更新時だけ処理が再実行される。*/
  computed: {
    ...
    taxedBasePrice: function() {
      ...
      //納期までの残り日数を計算
      var dateDiff = this.geetDateDiff(this.dalivery_date, (new Date().toLocaleString));
  ...
  created: function() {
    ...
    //DVD仕上がり予定日の初期値に、挙式日の初期値の1週間前の日付を設定
    dt.setDate(dt.getDate() - 7);
    dt.delivery_date = this.formatDate(dt);
  }
});
